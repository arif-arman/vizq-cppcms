...
function init() {
	...
	*/
	...
	mapA = textureLoader.load( "/textures/gmap_red.png");
	mapB = textureLoader.load( "/textures/gmap_green.png");
	...
	if (q == -1) {
		for (q = 0; q< querypoints.length; q++) {
			...
			*/
			...
			var visible_planes_ids = [false, false, false, false, false, false];
			for (var j=0;j<parseInt(querypoints[q]['num_visible_planes']);++j) {
				var planeid = parseInt(querypoints[q]['visible_planes'][j]['planeid']);
				visible_planes_ids[planeid] = true;
			}

			console.log(visible_planes_ids);
			
			//for (var j=0; j< Object.keys(querypoints[q]['visible_planes']).length; j++) {
			var vis_plane_index = 0;
			for (var j=0; j< 6; j++) {
				...
				//var pid = querypoints[q]['visible_planes'][j]['planeid'];
				var planeid = j;
				var p1 = cornerPoints[boxPlanes[planeid][0]];
				var p2 = cornerPoints[boxPlanes[planeid][1]];
				var p3 = cornerPoints[boxPlanes[planeid][2]];
				var p4 = cornerPoints[boxPlanes[planeid][3]];
				...
				if (visible_planes_ids[planeid] == true) {	// color it white
					var geometry = new THREE.Geometry();  
					geometry.vertices.push(
					...
					console.log(querypoints[q]['visible_planes'][vis_plane_index]['num_invisible_parts']);
					//for (var k=0; k<Object.keys(querypoints[q]['visible_planes'][j]['invisible_parts']).length; k++) {
					for (var k=0; k<parseInt(querypoints[q]['visible_planes'][vis_plane_index]['num_invisible_parts']); ++k) {
						var p1 = querypoints[q]['visible_planes'][vis_plane_index]['invisible_parts'][k]['boundary'][0].split(' ');
						var p2 = querypoints[q]['visible_planes'][vis_plane_index]['invisible_parts'][k]['boundary'][1].split(' ');
						var p3 = querypoints[q]['visible_planes'][vis_plane_index]['invisible_parts'][k]['boundary'][2].split(' ');
						var p4 = querypoints[q]['visible_planes'][vis_plane_index]['invisible_parts'][k]['boundary'][3].split(' ');
						for (var l=0; l<3; l++) {
			...
			if (q < k_ans) {
				// rank marker
				var textureLoader = new THREE.TextureLoader();
				var mapR = textureLoader.load( "/textures/numbers_blue/number_" + (q+1) + ".png");
	...
	get('/hello/mvq', params, function(text) {
		if (text != null) {	
			...
			for (var i=0;i<size;i++) {
				if (i==0) {
					for (var j=0;j<8;++j) {
						var p1 = json_obj[i]["corner_points"][j].split(' ');
						console.log(p1);
...
function loadTP() {
	...
	get('http://localhost:8000/read_file', params, function(text) {
		if (text == null) {
		}
		else {
			var json_obj = JSON.parse(text);
...
function runCMVQ() {
	...
	//console.log(params);
	get('http://localhost:8000/cmvq', params, function(text) {
		if (text != null) {	
			var json_obj = JSON.parse(text);
			console.log(json_obj);
			if (json_obj) {
...
function run() {
	...
	$('#logger').html('<img src="/gears.gif" width="50px" style="display: block; margin-left:auto; margin-right:auto;">');
