...
void RTree::Alternate_approach3D(QueryPoint3D q[], int num_of_query_points, Box2 T,int k, QueryPoint3D k_answers[], int& page,int& obstacle_considered, Point3D cornerPoints[])
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
void RTree::naiveMOV3D(QueryPoint3D q[], int num_of_query_points, Box2 T,int k, QueryPoint3D k_answers[],int& page, int& obstacle_considered, Point3D cornerPoints[])
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
