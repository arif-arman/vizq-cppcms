...
public:
	...
	string getVisiblePlaneBoundary(int j, int k) {
		if (k == 0) return visiblePlanes[j].boundary.p1.toStr();
		if (k == 1) return visiblePlanes[j].boundary.p2.toStr();
		if (k == 2) return visiblePlanes[j].boundary.p3.toStr();
		if (k == 3) return visiblePlanes[j].boundary.p4.toStr();
	}
