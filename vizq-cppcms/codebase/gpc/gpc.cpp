...
void gpc_copy_polygon( gpc_polygon     *p,gpc_polygon     *q ) //added by kzr
{

	int c, v;

	p->num_contours=q->num_contours;
	//printf("%d\n",(p->num_contours));
	MALLOC(p->hole, p->num_contours * sizeof(int),
			"hole flag array creation", int);
	MALLOC(p->contour, p->num_contours
	...
	for (c= 0; c < p->num_contours; c++)
	{
		p->contour[c].num_vertices=q->contour[c].num_vertices;
		p->hole[c]= FALSE; /* Assume all contours to be external */
		...
		for (v= 0; v < p->contour[c].num_vertices; v++)
		{
			p->contour[c].vertex[v].x=q->contour[c].vertex[v].x;
			p->contour[c].vertex[v].y=q->contour[c].vertex[v].y;
		}
...
int main()
{
    ...
    FILE *fp3=fopen("in1.txt", "r");
    FILE *fp2=fopen("out.txt", "w");
    FILE *fp4=fopen("out1.txt", "w");
  //  printf("asdasd");

    gpc_read_polygon(fp1,0,&p1);
    gpc_read_polygon(fp3,0,&p2);

    gpc_write_polygon(fp2,0,&p1);
    gpc_write_polygon(fp4,0,&p2);

    gpc_polygon p;

    gpc_polygon_clip(GPC_UNION,&p1,&p2,&p);

    FILE *fp5=fopen("out2.txt", "w");
    gpc_write_polygon(fp5,0,&p);
